...
import { SidebarChromeHeader } from "./layout/sidebar-chrome"
import { SidebarProfileFooter } from "./layout/sidebar-profile"
import { ShellBreadcrumb, ShellSidebarToggle } from "./layout/shell-nav"
...
export default function Layout(props: ParentProps) {
  ...
  const sidebarContent = (mobile?: boolean, railOnly?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("https://orgn.com/support")}
      renderPanel={() =>
  ...
  const sidebarChromeMac = () => platform.platform === "desktop" && platform.os === "macos"
  ...
  const desktopTitlebar = (
    <Titlebar update={titlebarUpdate} breadcrumb={titlebarBreadcrumb} unifiedSidebar class="hidden xl:flex shrink-0" />
  )

  const sidebarChrome = (mobile?: boolean) => (
    <SidebarChromeHeader
      mobile={mobile}
      mac={sidebarChromeMac()}
      zoom={titlebarZoom()}
      onToggleSidebar={handleShellSidebarToggle}
      toggleLabel={language.t("command.sidebar.toggle")}
      toggleKeybind={command.keybind("sidebar.toggle") ?? ""}
    />
  )
  ...
  return (
    <Show
      ...
      fallback={
        <div class="relative bg-v2-background-bg-deep flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
          {autoselecting() ?? ""}
          <div class="shrink-0 md:hidden">{mobileTitlebar}</div>
          ...
          <div class="flex-1 min-h-0 min-w-0 flex flex-row">
            <nav
              ...
              class="hidden md:flex shrink-0 flex-col overflow-hidden border-r border-border-weak-base bg-v2-background-bg-deep"
              classList={{ hidden: state.railCollapsed }}
            >
              {sidebarChrome()}
              <div class="@container min-h-0 flex-1 contain-strict overflow-hidden">{sidebarContent(false, true)}</div>
            </nav>
            <div class="flex min-h-0 min-w-0 flex-1 flex-col">
              <div class="hidden shrink-0 md:flex">{desktopTitlebar}</div>
    ...
    >
      <div class="relative bg-background-base flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
        {autoselecting() ?? ""}
        <div class="shrink-0 xl:hidden">{mobileTitlebar}</div>
        <Show when={updateVersion() !== undefined}>
          <UpdateAvailableToast version={updateVersion() ?? ""} install={installUpdate} language={language} />
        </Show>
          ...
          <div class="flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div class="relative min-h-0 flex-1">
              ...
              <Show when={layout.sidebar.opened()}>
                <div
                  class="hidden xl:block absolute inset-y-0 left-0 z-30 w-0 overflow-visible"
              ...
              <div class="xl:hidden">
                ...
                <nav
                  ...
                  classList={{
                    "@container fixed top-[47px] bottom-0 left-0 z-50 flex w-full max-w-[400px] flex-col overflow-hidden border-r border-border-weaker-base bg-background-base transition-transform duration-200 ease-out": true,
                ...
                >
                  {sidebarChrome(true)}
                  <div class="min-h-0 flex-1 overflow-hidden">{sidebarContent(true)}</div>
                </nav>
              </div>
