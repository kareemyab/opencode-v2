...
function useDirectoryStatusPanels() {
  const sync = useSync()
  const global = useGlobal()
  const server = useServer()
  const platform = usePlatform()
  ...
  return {
    ...
    pluginEmpty,
    manageServers,
    serverCount: () => global.servers.list().length,
    indexed: () => sync.data.mcp_ready,
  }
}

export function ServersDirectoryStatusPanel() {
  const global = useGlobal()
  const server = useServer()
  const language = useLanguage()
  const navigate = useNavigate()
  const panels = useDirectoryStatusPanels()

  return (
    <div class="flex flex-col px-2 pb-2">
      <div class="flex flex-col p-3 bg-background-base rounded-sm min-h-14">
        <For each={panels.sortedServers()}>
          {(s) => {
            ...
            return (
              ...
              >
                ...
                <ServerRow
                  ...
                  versionClass="text-12-regular text-text-weak truncate"
                  badge={
                    <Show when={key === panels.defaultServer.key()}>
                      <span class="text-11-regular text-text-base bg-surface-base px-1.5 py-0.5 rounded-md">
        ...
        <Button variant="secondary" class="mt-3 self-start h-8 px-3 py-1.5" onClick={panels.manageServers}>
            ...
            {(name) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (panels.toggleMcp.isPending) return
                    panels.toggleMcp.mutate(name)
                  }}
                  disabled={panels.toggleMcp.isPending && panels.toggleMcp.variables === name}
                >
                  <div
                    classList={{
...
export function PluginsStatusPanel() {
  const panels = useDirectoryStatusPanels()
  ...
  return (
    <div class="flex flex-col px-2 pb-2">
      <div class="flex flex-col p-3 bg-background-base rounded-sm min-h-14">
        <Show
          when={panels.plugins().length > 0}
          fallback={<div class="text-14-regular text-text-base text-center my-auto">{panels.pluginEmpty()}</div>}
        >
          <For each={panels.plugins()}>
            {(plugin) => (
              <div class="flex items-center gap-2 w-full px-2 py-1">
...
export function useDirectoryStatusCounts() {
  const panels = useDirectoryStatusPanels()
  return {
    serverCount: panels.serverCount,
    mcpConnected: panels.mcpConnected,
    mcpFailed: panels.mcpFailed,
    lspCount: panels.lspCount,
    pluginCount: panels.pluginCount,
    indexed: panels.indexed,
  }
}
      ...
      >
        <Tabs.List data-slot="tablist" class="bg-transparent border-b-0 px-4 pt-2 pb-0 gap-4 h-10">
          {!settings.general.newLayoutDesigns() && (
            <Tabs.Trigger value="servers" data-slot="tab" class="text-12-regular">
              {counts.serverCount() > 0 ? `${counts.serverCount()} ` : ""}
              {language.t("status.popover.tab.servers")}
            </Tabs.Trigger>
          )}
          <Tabs.Trigger value="mcp" data-slot="tab" class="text-12-regular">
            {counts.mcpConnected() > 0 ? `${counts.mcpConnected()} ` : ""}
            {language.t("status.popover.tab.mcp")}
          </Tabs.Trigger>
          <Tabs.Trigger value="lsp" data-slot="tab" class="text-12-regular">
            {counts.lspCount() > 0 ? `${counts.lspCount()} ` : ""}
            {language.t("status.popover.tab.lsp")}
          </Tabs.Trigger>
          <Tabs.Trigger value="plugins" data-slot="tab" class="text-12-regular">
            {counts.pluginCount() > 0 ? `${counts.pluginCount()} ` : ""}
