...
const lastThemeKeyByPty = new Map<string, string>()

const themeSessionKey = (theme: ReturnType<typeof useTheme>) =>
  `${theme.themeId()}:${theme.curatedKey()}:${theme.mode()}`
...
export const Terminal = (props: TerminalProps) => {
  ...
  const themeKey = createMemo(() => `${theme.themeId()}:${theme.curatedKey()}:${theme.mode()}`)

  return (
    <Show when={themeKey()} keyed>
      <TerminalView {...props} />
    </Show>
  )
}

const TerminalView = (props: TerminalProps) => {
  ...
  const sessionKey = themeSessionKey(theme)
  const previousSessionKey = lastThemeKeyByPty.get(id)
  const themeChanged = previousSessionKey !== undefined && previousSessionKey !== sessionKey
  lastThemeKeyByPty.set(id, sessionKey)

  const persisted = typeof local.pty.buffer === "string" ? local.pty.buffer : ""
  const restore = themeChanged ? "" : persisted
  const restoreSize =
    restore &&
  ...
  let cursor = themeChanged ? 0 : (start ?? 0)
  let seek = themeChanged ? 0 : start !== undefined ? start : restore ? -1 : 0
  ...
  return (
    <div
      ...
      tabIndex={-1}
      style={{
        "background-color": terminalTheme().background,
